  ...
  <div class="container mt-4">
    ...
    <div class="card">
      <div class="card-header bg-primary text-white">
        <h5 class="mb-0">Edit Kuis: {{ $quiz->title }}</h5>
      </div>
      <div class="card-body">
        <form action="{{ route('teacher.materials.quiz.update', $material->id) }}" method="POST" id="quizForm">
          @csrf
          @method('PUT')
          ...
          <div class="row mb-3">
            ...
            <div class="col-md-6">
              ...
              <input type="number" name="passing_score" id="passing_score" class="form-control" min="1" max="100" value="{{ old('passing_score', $quiz->passing_score) }}" required>
          ...
          <div class="mb-3">
            ...
            <textarea name="description" id="description" class="form-control" rows="2">{{ old('description', $quiz->description) }}</textarea>
          ...
          <div class="row mb-3">
            <div class="col-md-6">
              ...
              <input type="number" name="time_limit" id="time_limit" class="form-control" min="1" value="{{ old('time_limit', $quiz->time_limit) }}">
              <small class="text-muted">Kosongkan jika tidak ada batas waktu</small>
            </div>
            
            <div class="col-md-6 d-flex align-items-end">
              <div class="form-check me-4">
                <input class="form-check-input" type="checkbox" name="must_pass" id="must_pass" value="1" {{ old('must_pass', $quiz->must_pass) ? 'checked' : '' }}>
              ...
              <div class="form-check">
                <input class="form-check-input" type="checkbox" name="active" id="active" value="1" {{ old('active', $quiz->active) ? 'checked' : '' }}>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function addQuestion(questionData = null) {
        ...
        // Jika ada data pertanyaan, isi nilai-nilainya
        if (questionData) {
          newQuestion.querySelector('.question-text').value = questionData.text;
          newQuestion.querySelector('input[name^="questions"][name$="[points]"]').value = questionData.points || 1;
        }
        ...
        // Jika ada data pertanyaan, tambahkan pilihan yang sudah ada
        if (questionData && questionData.options && questionData.options.length > 0) {
          questionData.options.forEach((optionText, index) => {
            addOption(card, questionIdx, optionText);
          });
          
          // Set correct answer
          setTimeout(() => {
            const correctAnswerSelect = card.querySelector('.correct-answer-select');
            if (correctAnswerSelect && correctAnswerSelect.options.length > questionData.correct_answer) {
              correctAnswerSelect.value = questionData.correct_answer;
            }
          }, 100);
        } else {
          // Tambahkan 4 pilihan default jika tidak ada yang tersimpan
          for (let i = 0; i < 4; i++) {
      ...
      function addOption(questionCard, questionIdx, optionText = '') {
        ...
        // Isi nilai pilihan jika ada
        if (optionText) {
          newOption.querySelector('.option-text').value = optionText;
        }
        ...
        newOption.querySelector('.option-text').addEventListener('input', function() {
          updateCorrectAnswerOptions(questionCard, questionIdx);
        });
      ...
      document.getElementById('quizForm').addEventListener('submit', function(e) {
        ...
        questions.forEach((question, index) => {
          ...
          if (!correctAnswerSelect.value) {
            isValid = false;
            alert(`Pertanyaan ${index + 1} harus memiliki jawaban benar yang dipilih.`);
          }
        });
      ...
      if (existingQuestions && existingQuestions.length > 0) {
        ...
        // Tambahkan pertanyaan pertama jika tidak ada yang tersimpan
        addQuestion();
      }
    });
  </script>
